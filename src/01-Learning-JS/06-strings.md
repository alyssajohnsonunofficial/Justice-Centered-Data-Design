...
for (const punct of vonnegutNoSB) {
  if (punct == "\"") {
    vonnegutNoPuncs += ""
  }
  else if (punct == "\'") {
...
```javascript
/*
The oddity I noticed was that only the hyphens themselves were showing up; not the words that contained the hyphens. Perhaps I did this wrong? I'm not really sure what the issue is or how I could go about fixing it, unless I did something that would take longer, like doing a chain of .push commands for every single hyphenated word in the text. Curious to see what other students have to say about this one and about possible solutions. 
*/
